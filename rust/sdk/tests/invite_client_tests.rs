...
use phoenix_rise::accounts::trader::capabilities::TRADER_CAPABILITY_CAN_DEPOSIT;
...
fn referral_activation_trader_status_tracks_deposit_capability() {
    ...
    let active_flags = TRADER_CAPABILITY_CAN_DEPOSIT;
    assert_eq!(
        referral_activation_trader_status(Some(active_flags)),
        ReferralActivationTraderStatus::Activated
    );
    assert!(has_referral_activation_capabilities(active_flags));

    assert_eq!(
        referral_activation_trader_status(Some(0)),
        ReferralActivationTraderStatus::Registered
    );
    assert!(!has_referral_activation_capabilities(0));
}
