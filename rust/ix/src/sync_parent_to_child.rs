...
use crate::types::{AccountMeta, Instruction, push_writable_accounts};
...
pub fn create_sync_parent_to_child_ix(
    ...
    let data = crate::PhoenixInstruction::SyncParentToChild
        .discriminant()
        .to_vec();
...
fn build_accounts(params: &SyncParentToChildParams) -> Vec<AccountMeta> {
    ...
    push_writable_accounts(&mut accounts, params.global_trader_index());
...
mod tests {
    ...
    fn test_create_sync_parent_to_child_ix() {
        ...
        assert_eq!(
            &ix.data[..8],
            &crate::PhoenixInstruction::SyncParentToChild.discriminant()
        );
    }
