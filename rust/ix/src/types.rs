...
pub(crate) fn push_writable_accounts(accounts: &mut Vec<AccountMeta>, pubkeys: &[Pubkey]) {
    accounts.extend(pubkeys.iter().copied().map(AccountMeta::writable));
}

pub(crate) fn push_trader_index_accounts(
    accounts: &mut Vec<AccountMeta>,
    global_trader_index: &[Pubkey],
    active_trader_buffer: &[Pubkey],
) {
    push_writable_accounts(accounts, global_trader_index);
    push_writable_accounts(accounts, active_trader_buffer);
}
