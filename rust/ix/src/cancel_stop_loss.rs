...
fn encode_cancel_stop_loss(params: &CancelStopLossParams) -> Vec<u8> {
    ...
    data.extend_from_slice(&crate::PhoenixInstruction::CancelStopLoss.discriminant());
...
fn build_accounts(params: &CancelStopLossParams) -> Result<Vec<AccountMeta>, PhoenixIxError> {
...
mod tests {
    ...
    fn test_create_cancel_stop_loss_ix() {
        ...
        assert_eq!(
            &ix.data[..8],
            &crate::PhoenixInstruction::CancelStopLoss.discriminant()
        );
    }
    ...
    fn test_cancel_stop_loss_account_positions() {
        let params = test_params();
        let accounts = build_accounts(&params).unwrap();
        ...
        let expected_sl_pda =
            get_stop_loss_address(&params.trader_account, params.asset_id).unwrap();
