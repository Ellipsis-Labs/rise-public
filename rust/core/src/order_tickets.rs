...
use phoenix_rise_api::types::ix::TpSlOrderConfig;
use phoenix_rise_api::{CROSS_MARGIN_SUBACCOUNT_IDX, ExchangeKeysView, ExchangeMarketConfig};
use phoenix_rise_ix::limit_order::LimitOrderParams;
use phoenix_rise_ix::market_order::MarketOrderParams;
use phoenix_rise_ix::types::{OrderFlags, SelfTradeBehavior, Side, StopLossOrderKind};
use phoenix_rise_math::MarketCalculator;
use solana_pubkey::Pubkey;
use solana_rpc_client::nonblocking::rpc_client::RpcClient;

use crate::tx_builder::{
    ParsedAddresses, PhoenixTxBuilderError, parse_active_trader_buffer_pubkeys,
    parse_global_trader_index_pubkeys,
};

const BPS_DENOMINATOR: f64 = 10_000.0;
/// Default slippage used when an IOC bracket leg omits an explicit execution
/// price.
pub const DEFAULT_BRACKET_LEG_SLIPPAGE_BPS: u32 = 1_000;

/// Metadata required to convert an order ticket into ix-level params.
///
/// Call [`PhoenixTxBuilder::order_ticket_metadata`](crate::PhoenixTxBuilder::order_ticket_metadata)
/// when manually converting [`MarketOrderTicket`] or [`LimitOrderTicket`] into
/// raw ix params.
...
impl OrderTicketMetadata<'_> {
    fn parse_addresses(&self) -> Result<ParsedAddresses, PhoenixTxBuilderError> {
        Ok(ParsedAddresses {
            ...
            global_trader_index: parse_global_trader_index_pubkeys(
                &self.exchange_keys.global_trader_index,
            )?,
            active_trader_buffer: parse_active_trader_buffer_pubkeys(
                &self.exchange_keys.active_trader_buffer,
            )?,
...
///
/// This is the ergonomic input type for
/// [`PhoenixTxBuilder::place_market_order`](crate::PhoenixTxBuilder::place_market_order).
/// It accepts UI-level concepts such as symbol and optional price, then
/// [`Self::to_params`] resolves market metadata into raw
/// [`MarketOrderParams`].
#[derive(Debug, Clone)]
...
impl MarketOrderTicket {
    ...
    /// Convert this ticket into raw market-order params using resolved market
    /// metadata.
    ///
    /// Callers usually let
    /// [`PhoenixTxBuilder::place_market_order`](crate::PhoenixTxBuilder::place_market_order)
    /// do this. Use this method when you need to inspect or modify the ix
    /// params before calling `phoenix-rise-ix` builders directly.
    pub fn to_params(
        &self,
...
impl MarketOrderTicketBuilder {
    ...
    pub fn build(self) -> Result<MarketOrderTicket, phoenix_rise_ix::PhoenixIxError> {
        Ok(MarketOrderTicket {
            authority: self
                .authority
                .ok_or(phoenix_rise_ix::PhoenixIxError::MissingField("authority"))?,
            trader_account: self.trader_account.ok_or(
                phoenix_rise_ix::PhoenixIxError::MissingField("trader_account"),
            )?,
            symbol: self
                .symbol
                .ok_or(phoenix_rise_ix::PhoenixIxError::MissingField("symbol"))?,
            side: self
                .side
                .ok_or(phoenix_rise_ix::PhoenixIxError::MissingField("side"))?,
            price: self.price,
            num_base_lots: self.num_base_lots.ok_or(
                phoenix_rise_ix::PhoenixIxError::MissingField("num_base_lots"),
            )?,
...
///
/// This is the ergonomic input type for
/// [`PhoenixTxBuilder::place_limit_order`](crate::PhoenixTxBuilder::place_limit_order).
/// It accepts a UI price and converts it to ticks through
/// [`Self::to_params`].
#[derive(Debug, Clone)]
...
impl LimitOrderTicket {
    ...
    /// Convert this ticket into raw limit-order params using resolved market
    /// metadata.
    ///
    /// Callers usually let
    /// [`PhoenixTxBuilder::place_limit_order`](crate::PhoenixTxBuilder::place_limit_order)
    /// do this. Use this method when you need raw [`LimitOrderParams`] for
    /// custom instruction batching.
    pub fn to_params(
        &self,
        metadata: OrderTicketMetadata<'_>,
...
impl LimitOrderTicketBuilder {
    ...
    pub fn build(self) -> Result<LimitOrderTicket, phoenix_rise_ix::PhoenixIxError> {
        Ok(LimitOrderTicket {
            authority: self
                .authority
                .ok_or(phoenix_rise_ix::PhoenixIxError::MissingField("authority"))?,
            ...
            price: self
                .price
                .ok_or(phoenix_rise_ix::PhoenixIxError::MissingField("price"))?,
            num_base_lots: self.num_base_lots.ok_or(
                phoenix_rise_ix::PhoenixIxError::MissingField("num_base_lots"),
            )?,
            self_trade_behavior: self.self_trade_behavior.unwrap_or(SelfTradeBehavior::Abort),
...
impl BracketLeg {
    /// Create a bracket leg with a trigger price in UI price units.
    ///
    /// Defaults are resolved by the transaction builder: stop-loss legs default
    /// to IOC execution and take-profit legs default to limit execution.
    ...
    /// Set an explicit execution price in UI price units.
    ...
    /// Derive the execution price from the trigger price and primary order
    /// side using basis-point slippage.
...
pub struct BracketLegTicket {
    ...
    conditional_orders_account_mode: ConditionalOrdersAccountMode,
}

/// How the tx builder should handle the trader conditional-orders account
/// before appending bracket-leg instructions.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ConditionalOrdersAccountMode {
    /// Query RPC and prepend a create instruction if the account is missing.
    CheckAndCreateIfMissing,
    /// Do not query RPC. Use this when the account is already known to exist
    /// or account creation is handled in a separate instruction bundle.
...
impl std::fmt::Debug for BracketLegTicket {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BracketLegTicket")
            ...
            .field(
                "conditional_orders_account_mode",
                &self.conditional_orders_account_mode,
            )
            .finish()
    }
}
...
impl BracketLegTicket {
    /// Create bracket configuration that can be attached to an order ticket.
    ///
    /// The RPC client is used only when the builder may need to create or
    /// verify a conditional-orders account before appending bracket ixs.
    pub fn new(rpc_client: Arc<RpcClient>, bracket_legs: BracketLegOrders) -> Self {
        Self {
            rpc_client,
            bracket_legs,
            conditional_orders_account_mode: ConditionalOrdersAccountMode::CheckAndCreateIfMissing,
        }
    }

    /// Skip the builder's RPC check for the conditional-orders account.
    ///
    /// Use this when another instruction in the same bundle creates the
    /// account, or when your application has already verified that it exists
    /// against the same RPC/commitment used for submission.
    pub fn without_conditional_orders_account_check(mut self) -> Self {
        self.conditional_orders_account_mode = ConditionalOrdersAccountMode::SkipPrerequisiteCheck;
        self
    }

    /// Re-enable the default RPC check and create-if-missing behavior.
    pub fn with_conditional_orders_account_check(mut self) -> Self {
        self.conditional_orders_account_mode =
    ...
    pub fn conditional_orders_account_mode(&self) -> ConditionalOrdersAccountMode {
        self.conditional_orders_account_mode
    }
}
