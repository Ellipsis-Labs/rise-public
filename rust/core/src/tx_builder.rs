...
use crate::bracket_pricing::{BracketLegExecutionPriceError, bracket_leg_execution_ticks};
use crate::order_tickets::{
    BracketLegOrders, BracketLegSize, BracketLegTicket, ConditionalOrdersAccountMode,
    LimitOrderTicket, MarketOrderTicket, OrderTicketMetadata,
};
use crate::{
    CROSS_MARGIN_SUBACCOUNT_IDX, ExchangeMarketConfig, PhoenixMetadata, Trader, TraderKey,
};

const USDC_NATIVE_DECIMALS: f64 = 1_000_000.0;
const DEFAULT_CONDITIONAL_ORDERS_CAPACITY: u8 = 8;
const MAX_METADATA_ACCOUNT_KEYS: usize = 256;
...
pub enum PhoenixTxBuilderError {
    ...
    Instruction(#[from] phoenix_rise_ix::PhoenixIxError),
    ...
    InvalidBracketLegExecutionPrice(BracketLegExecutionPriceError),
    ...
    /// Exchange metadata contains more dynamic accounts than a single Solana
    /// transaction can reasonably carry.
    #[error("{label} has {count} accounts, which exceeds the maximum of {max}")]
    TooManyMetadataAccounts {
        label: &'static str,
        count: usize,
        max: usize,
    },
...
impl From<BracketLegExecutionPriceError> for PhoenixTxBuilderError {
    fn from(error: BracketLegExecutionPriceError) -> Self {
        Self::InvalidBracketLegExecutionPrice(error)
    }
}
...
///
/// ```no_run
/// use phoenix_rise_api::PhoenixHttpClient;
/// use phoenix_rise_core::{MarketOrderTicket, PhoenixMetadata, PhoenixTxBuilder};
...
impl<'a> PhoenixTxBuilder<'a> {
    ...
    ///
    /// Most callers use [`Self::place_market_order`] or
    /// [`Self::place_limit_order`] directly. Call this when converting a ticket
    /// manually with `MarketOrderTicket::to_params` or
    /// `LimitOrderTicket::to_params`.
    ...
    async fn create_bracket_market_order_ixs(
        ...
        let mut ixs = self
            .conditional_orders_account_ixs(authority, trader_account, bracket)
            .await?;
    ...
    async fn create_bracket_limit_order_ixs(
        ...
        let mut ixs = self
            .conditional_orders_account_ixs(params.trader(), params.trader_account(), bracket)
            .await?;
    ...
    pub async fn build_cancel_bracket_leg_from_account(
        ...
        let stop_losses_trader_key = Pubkey::new_from_array(stop_losses.trader_key());
        if stop_losses_trader_key != trader_pda {
            return Err(PhoenixTxBuilderError::Rpc(format!(
                "stop loss account {stop_loss} belongs to trader {}, expected {trader_pda}",
                stop_losses_trader_key
            )));
        }
        if u64::from(stop_losses.asset_id()) != asset_id {
            return Err(PhoenixTxBuilderError::Rpc(format!(
        ...
        self.build_cancel_bracket_leg_with_funder(
            Pubkey::new_from_array(stop_losses.funding_key()),
            authority,
            trader_pda,
            symbol,
            execution_direction,
    ...
    pub fn build_deposit_funds(
        ...
        let global_trader_index = parse_global_trader_index_pubkeys(&keys.global_trader_index)?;
    ...
    /// Build an atomic withdraw-and-convert flow.
    ///
    /// This method builds the full withdrawal flow for withdrawals that are
    ...
    ///
    /// If a Phoenix withdrawal can be queued, prefer
    /// [`Self::build_request_withdraw_funds`] first and submit
    /// [`Self::build_complete_withdraw_funds`] only after the Phoenix tokens
    /// have landed. Splitting the flow avoids a transaction that appears to
    /// request a withdrawal but cannot complete the Ember conversion yet.
    ///
    /// # Arguments
    ///
    /// * `authority` - The trader's wallet address (signer)
    ...
    pub fn build_withdraw_funds(
        ...
        // Convert USDC amount to base units (6 decimals)
        let amount = (usdc_amount * 1_000_000.0) as u64;
        self.build_withdraw_funds_base_units(authority, trader_pda, amount)
    }

    /// Build only the Phoenix withdrawal request phase.
    ///
    /// Use this for queued withdrawals. After the withdrawal is processed and
    /// the authority's Phoenix collateral token account receives funds, call
    /// [`Self::build_complete_withdraw_funds`] to convert those tokens back to
    /// USDC through Ember.
    pub fn build_request_withdraw_funds(
        &self,
        authority: Pubkey,
        trader_pda: Pubkey,
        usdc_amount: f64,
    ) -> Result<Vec<Instruction>, PhoenixTxBuilderError> {
        let amount = (usdc_amount * USDC_NATIVE_DECIMALS) as u64;
        self.build_request_withdraw_funds_base_units(authority, trader_pda, amount)
    }

    /// Build only the Phoenix withdrawal request phase using canonical token
    /// base units.
    pub fn build_request_withdraw_funds_base_units(
        &self,
        authority: Pubkey,
        trader_pda: Pubkey,
        amount: u64,
        ...
        Ok(vec![create_phoenix_ata_ix.into(), withdraw_ix.into()])
    }

    /// Build only the Ember conversion phase for a completed Phoenix
    /// withdrawal.
    pub fn build_complete_withdraw_funds(
        &self,
        authority: Pubkey,
        usdc_amount: f64,
    ) -> Result<Vec<Instruction>, PhoenixTxBuilderError> {
        let amount = (usdc_amount * USDC_NATIVE_DECIMALS) as u64;
        self.build_complete_withdraw_funds_base_units(authority, amount)
    }

    /// Build only the Ember conversion phase using canonical token base units.
    pub fn build_complete_withdraw_funds_base_units(
        &self,
        authority: Pubkey,
        amount: u64,
    ) -> Result<Vec<Instruction>, PhoenixTxBuilderError> {
    ...
    /// Build withdraw funds instructions using canonical token base units.
    ///
    /// This is the precision-safe variant of [`Self::build_withdraw_funds`].
    /// Amounts are in base units (for USDC-like canonical tokens, 1 USDC =
    /// 1,000,000 base units).
    ///
    /// This preserves the legacy atomic flow. For queued withdrawals, split the
    /// flow with [`Self::build_request_withdraw_funds_base_units`] and
    /// [`Self::build_complete_withdraw_funds_base_units`].
    pub fn build_withdraw_funds_base_units(
        &self,
        authority: Pubkey,
        trader_pda: Pubkey,
        amount: u64,
    ...
    pub fn build_isolated_market_order_with_params(
        ...
        let (sub_key, ixs) = self.prepare_isolated_subaccount(
            trader,
            symbol,
        ...
        self.finish_isolated_market_order(trader, symbol, params, sub_key, ixs, bracket)
    }

    /// Build an isolated market order with bracket legs that can verify or
    /// create the conditional-orders account before appending bracket ixs.
    pub async fn build_isolated_market_order_with_bracket_ticket(
        &self,
        trader: &Trader,
        symbol: &str,
        params: IsolatedMarketOrderParams,
        bracket: Option<&BracketLegTicket>,
    ) -> Result<Vec<Instruction>, PhoenixTxBuilderError> {
        ...
        let bracket_legs = bracket
            .filter(|bracket| !bracket.is_empty())
            .map(|bracket| bracket.bracket_legs());
        if let Some(bracket) = bracket.filter(|bracket| !bracket.is_empty()) {
            ixs.extend(
                self.conditional_orders_account_ixs(sub_key.authority(), sub_key.pda(), bracket)
                    .await?,
            );
        }

        self.finish_isolated_market_order(trader, symbol, params, sub_key, ixs, bracket_legs)
    }

    fn finish_isolated_market_order(
        &self,
        trader: &Trader,
        symbol: &str,
        params: IsolatedMarketOrderParams,
    ...
    /// This is the lower-level helper behind order-ticket bracket support; use
    /// it directly when you already have a primary position and want to attach
    /// conditionals without placing a new order first.
    ///
    /// Direction logic:
    ...
    async fn conditional_orders_account_ixs(
        &self,
        authority: Pubkey,
        trader_account: Pubkey,
        bracket: &BracketLegTicket,
    ) -> Result<Vec<Instruction>, PhoenixTxBuilderError> {
        match bracket.conditional_orders_account_mode() {
            ConditionalOrdersAccountMode::CheckAndCreateIfMissing => {
                self.maybe_create_conditional_orders_account_ixs(
                    authority,
                    trader_account,
                    bracket.rpc_client(),
                )
            ...
            ConditionalOrdersAccountMode::SkipPrerequisiteCheck => Ok(Vec::new()),
        }
    }
    ...
    fn hawkeye_trader_accounts(
        ...
        Ok(HawkeyeTraderViewAccounts {
            ...
            global_trader_index: parse_global_trader_index_pubkeys(&keys.global_trader_index)?,
            active_trader_buffer: parse_active_trader_buffer_pubkeys(&keys.active_trader_buffer)?,
...
pub(crate) fn parse_global_trader_index_pubkeys(
    strings: &[String],
) -> Result<Vec<Pubkey>, PhoenixTxBuilderError> {
    parse_metadata_pubkey_vec("global_trader_index", strings)
}

pub(crate) fn parse_active_trader_buffer_pubkeys(
    strings: &[String],
) -> Result<Vec<Pubkey>, PhoenixTxBuilderError> {
    parse_metadata_pubkey_vec("active_trader_buffer", strings)
}

/// Parse a bounded vector of base58-encoded pubkeys from exchange metadata.
pub(crate) fn parse_metadata_pubkey_vec(
    label: &'static str,
    strings: &[String],
) -> Result<Vec<Pubkey>, PhoenixTxBuilderError> {
    if strings.len() > MAX_METADATA_ACCOUNT_KEYS {
        return Err(PhoenixTxBuilderError::TooManyMetadataAccounts {
            label,
            count: strings.len(),
            max: MAX_METADATA_ACCOUNT_KEYS,
        });
    }
...
mod tests {
    ...
    use phoenix_rise_ix::PhoenixInstruction;
    use phoenix_rise_ix::constants::{
        PHOENIX_GLOBAL_CONFIGURATION, PHOENIX_LOG_AUTHORITY, get_ember_vault_address,
    };
    use phoenix_rise_ix::types::OrderFlags;
    use phoenix_rise_math::{MarketCalculator, QuoteLotsPerBaseLotPerTick};
    ...
    fn test_parse_pubkey_vec() {
        ...
        let result = parse_global_trader_index_pubkeys(&pubkeys).unwrap();
    ...
    fn test_parse_pubkey_vec_invalid() {
        ...
        let result = parse_global_trader_index_pubkeys(&pubkeys);
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_pubkey_vec_rejects_unbounded_metadata() {
        let pubkeys = vec!["11111111111111111111111111111112".to_string(); 257];
        let result = parse_active_trader_buffer_pubkeys(&pubkeys);
        assert!(matches!(
            result,
            Err(PhoenixTxBuilderError::TooManyMetadataAccounts {
                label: "active_trader_buffer",
                count: 257,
                max: 256
            })
        ));
    ...
    fn test_builder_ignores_metadata_program_id_for_ember_accounts() {
        ...
        assert_eq!(
            ...
            get_ember_state_address().unwrap()
        );
        assert_eq!(
            ixs[1].accounts[6].pubkey,
            get_ember_vault_address().unwrap()
        );
    ...
    fn test_builder_ignores_metadata_program_id_for_legacy_stop_loss() {
        ...
        for ix in ixs {
            ...
            assert_eq!(
                ...
                get_stop_loss_address(&trader_pda, 1).unwrap()
            );
        }
    }
