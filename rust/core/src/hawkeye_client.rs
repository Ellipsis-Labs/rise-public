...
use phoenix_rise_ix::HAWKEYE_PROGRAM_ID;
...
const DEFAULT_HAWKEYE_SIMULATION_FEE_PAYER: Pubkey =
    solana_pubkey::pubkey!("9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM");
const DEFAULT_HAWKEYE_SIMULATION_COMPUTE_UNIT_LIMIT: u32 = 1_400_000;
...
impl<'a> PhoenixHawkeyeClient<'a> {
    pub fn new(rpc_url: impl Into<String>, metadata: &'a PhoenixMetadata) -> Self {
        Self {
            ...
            fee_payer: DEFAULT_HAWKEYE_SIMULATION_FEE_PAYER,
            compute_unit_limit: DEFAULT_HAWKEYE_SIMULATION_COMPUTE_UNIT_LIMIT,
        }
    }
...
fn only_instruction(
    ...
    if actual != 1 {
        return Err(PhoenixHawkeyeClientError::UnexpectedInstructionCount { actual });
    }
    instructions
        .into_iter()
        .next()
        .ok_or(PhoenixHawkeyeClientError::UnexpectedInstructionCount { actual: 0 })
}
