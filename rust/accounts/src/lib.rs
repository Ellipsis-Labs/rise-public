...
//! prices, funding, risk, and spline account references. Use [`trader::Trader`]
//! for a zero-copy full trader view over account data, or
//! [`trader::TraderHeader`] when only fixed trader state, capability flags, and
//! preference flags are needed. Programs can use
//! [`permission::PermissionAccount::try_from_buffer`] for a direct zero-copy
//! permission account reference. Trader capability and preference helpers live
//! under [`trader::capabilities`] and [`trader::preferences`]. PDA helpers are
...
pub mod prelude {
    ...
    pub use super::trader::{
        Trader, TraderHeader, TraderPosition, TraderPositionEntry, TraderPositionEntryRef,
        TraderPositionIter, TraderPositionMapHeader, TraderPositionRef, TraderPositions,
        TraderState,
    };
