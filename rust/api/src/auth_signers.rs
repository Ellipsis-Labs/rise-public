...
impl PhoenixAuthSignerKind {
    ...
    fn parse(kind: &str) -> Result<Self, AuthError> {
        match kind {
            "ed25519" | "ed25519-dalek" | "ed25519-service" => Ok(Self::Ed25519ServiceAccount),
            "solana-service" => Ok(Self::SolanaServiceAccount),
            "solana-keypair" => Ok(Self::SolanaKeypair),
...
fn format_env_pair(primary: &'static str, fallback: &'static str) -> String {
    format!("{primary} (or {fallback})")
}
#[derive(Clone)]
