...
pub(crate) struct PhoenixApiClient {
    ...
    auth_refresh_lock: Arc<AsyncMutex<()>>,
}
#[cfg(feature = "opentelemetry")]
...
impl PhoenixApiClient {
    ...
    async fn refresh_session(&self) -> Result<(), PhoenixApiError> {
        let refresh_candidate = self
            .current_auth_session()
            .await?
            .map(|session| session.snapshot());
        let _guard = self.auth_refresh_lock.lock().await;
    ...
    async fn refresh_session_locked(&self) -> Result<(), PhoenixApiError> {
        if let Some(manager) = &self.session_manager {
...
impl PhoenixApiClientBuilder {
    ...
    pub(crate) fn build(self) -> Result<PhoenixApiClient, PhoenixApiError> {
        ...
        Ok(PhoenixApiClient {
            ...
            auth_refresh_lock: Arc::new(AsyncMutex::new(())),
        })
    }
}
...
mod tests {
    ...
    async fn concurrent_expired_access_requests_share_one_refresh() {
        let (base_url, requests) = spawn_auth_test_server(RefreshResponse::Success, 4);
        let client = Arc::new(
            PhoenixApiClient::builder(base_url)
                .with_auth_session(expired_session())
                .build()
                .expect("client should build"),
        );
            ...
            .map(|_| {
                ...
                tokio::spawn(async move {
                    client
                        ...
                        .await
                        .expect("protected request should succeed after shared refresh")
                })
            })
            .collect::<Vec<_>>();

        for task in tasks {
            assert_eq!(task.await.expect("task should join"), json!({ "ok": true }));
        }

        let requests = requests.lock();
        let refresh_count = requests
            .iter()
            .filter(|request| request.path == "/v1/auth/refresh")
            .count();
        let protected_count = requests
            .iter()
            .filter(|request| request.path == "/v1/view/markets")
            .count();
