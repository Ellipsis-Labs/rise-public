...
impl Position {
    ...
    fn from_row(symbol: &str, row: &TraderStatePositionRow) -> Self {
        Self {
            ...
            position_sequence_number: row.position_sequence_number.parse().unwrap_or(0),
    ...
    pub fn to_trader_position(&self) -> TraderPosition {
        let accumulated_funding_for_active_position =
            SignedQuoteLotsI56::try_from(SignedQuoteLots::new(self.accumulated_funding_quote_lots))
                .unwrap_or_default();
...
impl LimitOrder {
    fn from_event(symbol: &str, event: &TraderStateMarketLimitOrderEvent) -> Self {
        Self {
            ...
            conditional_kind: event.conditional_kind.clone(),
...
impl Trader {
    ...
    ///
    /// Prefers the lowest-index subaccount with an existing position in this
    /// asset. Falls back to the empty isolated subaccount with the greatest
    /// collateral, using the lowest index as a deterministic tie-breaker.
    pub fn isolated_subaccount_for_asset(&self, symbol: &str) -> Option<&SubaccountState> {
        if let Some(s) = self
            .subaccounts
            .values()
            .filter(|s| s.subaccount_index > 0 && s.positions.contains_key(symbol))
            .min_by_key(|s| s.subaccount_index)
        {
            return Some(s);
        }
        self.subaccounts
            ...
            .max_by(|a, b| {
                a.collateral
                    .cmp(&b.collateral)
                    .then_with(|| b.subaccount_index.cmp(&a.subaccount_index))
            })
    }
