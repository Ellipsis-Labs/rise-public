...
describe("trade history types", () => {
  ...
  it("parses nullable fillId for trades_v2", () => {
    const withFillId = TradeHistoryV2ItemSchema.parse({
      ...
      parentTwapId: "future-parent-twap-format",
      timestamp: 1_745_238_400_000,
      slot: 377700000,
      slotIndex: 2442,
      eventIndex: 0,
    ...
    expect(withFillId.parentTwapId).toBe("future-parent-twap-format");
    expect(withoutFillId.fillId).toBeNull();
    expect(withoutFillId.parentTwapId).toBeNull();
  });

  it("rejects empty parent TWAP IDs for trades_v2", () => {
    expect(() =>
      TradeHistoryV2ItemSchema.parse({
        userId: 1,
        traderId: 2,
        traderPdaIndex: 0,
        subaccountIndex: 0,
