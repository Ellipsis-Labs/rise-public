...
import { getPlaceLimitOrderEncoder } from "./codec";
import type {
...
export const buildPlaceLimitOrderIx = (
  ...
  const data = getPlaceLimitOrderEncoder().encode(params.orderPacket);

  const accounts: PlaceLimitOrderAccounts = [
...
const validate = (params: PlaceLimitOrderParams) => {
  if (!params.trader) {
