...
import { getCancelUpToEncoder } from "./codec";
import type {
  CancelUpToAccounts,
  CancelUpToIx,
  CancelUpToParams,
...
export const buildCancelUpToIx = (params: CancelUpToParams): CancelUpToIx => {
  validate(params);
  const { programAddress, logAuthorityAddress, globalConfigurationAddress } =
    getPhoenixInstructionAddresses(params);

  const data = getCancelUpToEncoder().encode({
    side: params.side,
...
const validate = (params: CancelUpToParams) => {
  if (!params.trader) {
