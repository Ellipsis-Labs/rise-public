...
import { getCreateEscrowRequestEncoder, normalizeActions } from "./codec";
import type {
...
export const buildCreateEscrowRequestIx = (
  params: CreateEscrowRequestParams
): CreateEscrowRequestIx => {
  validate(params);
  ...
  const data = getCreateEscrowRequestEncoder().encode({
    senderPdaIndex: params.senderPdaIndex,
    senderSubaccountIndex: params.senderSubaccountIndex,
    receiverPdaIndex: params.receiverPdaIndex,
    receiverSubaccountIndex: params.receiverSubaccountIndex,
    lastValidSlot: params.lastValidSlot ?? null,
    actions,
  });

  const accounts: CreateEscrowRequestAccounts = [
    ...
    generateReadonlySignerAccount(params.senderWallet),
    generateReadonlyAccount(params.senderTraderAccount),
    generateWritableAccount(params.permissionAccount),
    generateReadonlyAccount(params.receiverWallet),
    generateReadonlyAccount(params.receiverTraderAccount),
...
const validate = (params: CreateEscrowRequestParams) => {
  ...
  if (params.actions.length > 4) {
    throw new Error("At most 4 actions are allowed");
  }
};
